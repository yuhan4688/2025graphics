...
void setup(){//把圖加入新專案裡
  ...
  foot1 = loadImage("foot1.png");//增加腳旋轉中心220.375
  foot2 = loadImage("foot2.png");//增加腳 旋轉中心260.372
}
float [] angleX= new float[10];//在3D的世界裏頭，我們的旋轉
float [] angleY= new float[10];
int ID=0;//一開始是頭的關節
ArrayList<String> lines=new ArrayList<String>();
void keyPressed(){
  if(key=='s'){//現在動作的所有關節的值
    String now="";//空字串
    for(int i=0;i<10;i++){
      now+=angleX[i]+" ";//後面有空格
      now+=angleY[i]+" ";//後面有空格
