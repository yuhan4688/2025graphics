//week11-2_postman_saveStrings_loadString_push_trt_pop
  ...
  if(key=='s'){ // 從這裡開始 每次按 's' 就存一組 angle
    ...
    if(key=='r'){//replay
      if(R==0){//如果目前沒有任何東西
        ...
        if(file!=null){//如果有獨到檔案
          for(int i=0;i<file.length;i++){//執行檔案內容
            lines.add(file[i]);  //家道lines資料結構裡
          }
        }
      }
...
void myInterporlate(){
    if(lines.size()>0){ //按了 r之後,想要有內插的結果
      float alpha=(frameCount%30)/30.0;
      if(alpha==0.0) R=(R+1)%lines.size();
      int R2=(R+1)%lines.size(); 
      float[] oldAngle=float(split(lines.get(R),' '));
