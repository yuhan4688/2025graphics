//week10-6_postman_body_uparm1_uparm2_key_saveString_loadString_push_trt_pop
//頭、身體、手臂、手肘
...
void keyPressed(){
  ...
  if(key=='6') ID=6;
  if(key=='0') ID=0;
  if(key=='s'){//從這理開始每一次按's'就存一組angle
    String now="";
    for(int i=0;i<20;i++){
      now+=angle[i]+" ";//全部加到now裡，記得有空格
    }
    lines.add(now);
...
int R=0;
ArrayList<String> lines=new ArrayList<String>();//放移動字串
//到這裡結束
void setup(){
  size(560,560);
